...
public class GameState implements Cloneable {
	 ...
	 */
	void findPossibleMoves(Vector<Move> lMoves) {
	    ...
	    if (mMovesUntilDraw <= 0) {
	    	lMoves.add(new Move(Move.MoveType.MOVE_DRAW));
	    	return;
	    } // End if
		...
		if (lMoves.size() == 0) {
			lMoves.add(new Move(mNextPlayer == Constants.CELL_WHITE ?
					Move.MoveType.MOVE_RW : Move.MoveType.MOVE_WW));
	    }
